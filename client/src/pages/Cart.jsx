...
const Cart = () => {
    ...
    const { axios, product, currency, cartItems, removeFromCart, getCartCount, updateCartItem, navigate, getCartAmount, user, setCartItems } = useAppContext();
...
const getCart = () => {
    ...
    const placeOrder = async () => { // this is the part of backend
        try {
            if (!selectedAddress) {
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                <h1 className="text-3xl font-medium mb-6">
                    Shopping Cart <span className="text-sm text-red-500">{getCartCount()} Items</span>
                </h1>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button className="cursor-pointer mx-auto">
                            <img src={assets.remove_icon} onClick={() => removeFromCart(product._id)} alt="remove" className="inline-block w-6 h-6" srcset="" />
                        </button>
                    </div>)
                )}

                <button onClick={() => { navigate("/products"); scrollTo(0, 0) }} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    <img src={assets.arrow_right_icon_colored} alt="arrow" className="group-hover:- translate-x-1 transition" srcset="" />
                    Continue Shopping
                </button>
